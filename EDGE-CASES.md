...
| A share code encoding an absurd cycle | 10,000-day cycle stored | **Confirmed by reading, Phase 14:** there is no ceiling. `Pattern` requires only a non-empty cycle and `importSharedPattern` checks nothing, so the builder's 40-day limit does not apply to an imported code. Harm is low — every consumer resolves per day rather than walking the cycle, so nothing blows up, and the preview and the confirm step both stand in the way | **Import path** — apply the builder's ceiling; still v1.1 |
...
| **Device timezone changes** (travel; a phone set up abroad) | "Today" shifts by a day; the ringed cell is wrong | Rota dates are zone-free, so the *rota* is unaffected — only "which day is today" moves, which is arguably correct. The **ringed cell is now right**: the same Phase 14 receiver takes `ACTION_TIMEZONE_CHANGED`, and a timezone shift across the date line is how the midnight fix was tested. **Reminders are still not rescheduled on a timezone change** | **Scheduler** — half done. The alarm half remains; ranking #7 |
| **Device clock set wrong or manually changed** | Alarms fire at the wrong time or not at all | **The screens** now detect it — `ACTION_TIME_CHANGED` is in the Phase 14 filter, so the ringed day corrects itself. **The alarms still do not**: Android reschedules nothing on a time change, and neither does this app, so an already-set alarm keeps its old trigger time | v1.1 — the same scheduler fix as the row above |
